...
Future<Message> unregister(Message message, TeleDart teleDart) async {
  ...
  try {
    ...
    if (!registered) {
      return teleDart.replyMessage(message, 'Hey @${message.from.username} you were not registered :|');
    }

    //encoding back the user to the file
    await io.File(teamFile.path).writeAsStringSync(jsonEncode(team));
    return teleDart.replyMessage(message, '@${message.from.username} you have been un-registered from this weeks challenge.');
  } catch (e) {
    print(e.toString());
